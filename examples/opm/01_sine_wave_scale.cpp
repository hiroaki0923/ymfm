...
void playSineWaveScale(ymfm::ym2151& chip, std::vector<int16_t>& audioDataRaw, double YM2151_OUTPUT_RATE) {
    std::cout << "Playing sine wave scale..." << std::endl;
    
    // MIDI note numbers for Do-Re-Mi-Fa-So-La-Si-Do (from C4)
    ...
    for (int i = 0; i < NOTE_COUNT; i++) {
        ...
        std::cout << "Note " << i << ": KC=0x" << std::hex << (int)kc << ", KF=0x" << (int)(kf << 2) << std::dec << std::endl;
        ...
        for (int s = 0; s < samples_raw; s++) {
            ...
            if (s < 5 && i == 0) {
                std::cout << "Sample " << s << ": L=" << output.data[0] << ", R=" << output.data[1] << std::endl;
            }
